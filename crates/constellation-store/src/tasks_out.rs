...
pub fn get(store: &Store, task_id: &str) -> Result<Option<OutTask>> {
    let raw = store.with_conn(|conn| {
        ...
        if let Some(row) = rows.next()? {
            ...
            Ok(Some((task_id, to_peer, state, request_json, response_json, updated_at_str)))
        } else {
            Ok(None)
        }
    })?;
    let Some((task_id, to_peer, state, request_json, response_json, updated_at_str)) = raw else {
        return Ok(None);
    };
